...
class Births(ss.Demographics):
    ...
    def finalize(self):
        ...
        births_per_year = self.results.new / self.sim.t.dt_year
        denom = self.sim.results.n_alive
        self.results.cbr[:] = inv_rate_units * np.divide(births_per_year, denom, where=denom > 0)
