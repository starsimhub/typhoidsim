...
the intervention (e.g., campaign that finds eligible people) and also products.
"""
import numpy as np
...
# Interventions that are not treatments but change some of the agent's properties
...
class acute_treatment(ss.Intervention):
    ...
    def init_pre(self, sim):
        super().init_pre(sim)
        self.define_results(
            ss.Result('n_treated', dtype=int, label='Number treated today')  # count how many were treated today, includes new and old patients
        )
        return
...
class infection_clearence(ss.Intervention):
    ...
    def init_pre(self, sim):
        super().init_pre(sim)
        self.define_results(
            ss.Result('n_treated', dtype=int, label="Number treated today"),
            ss.Result('n_started_tr', dtype=int, label="Number started treatment today")
        )
        return
...
class base_test(ss.Intervention):
    ...
    def __init__(self, prob_t=1.0, prob_tp=1.0, eligibility=None, eligibility_kwargs=None, **kwargs):
        ...
        self.prob_tp = sc.promotetoarray(prob_tp)  # given that a person if being tested, probability of testing positive (capture uncertainty of test product/method)
        self.eligibility = eligibility
    ...
    def step(self):
        ...
        # Of those who were tested and are infected
        infected_uids = sim.people.typhoid.infected.uids
...
class WASH(ss.Intervention):
    ...
    def init_pre(self, sim):
        ...
        self.time = sc.inclusiverange(0, self.dur, sim.pars.dt)
        self.define_results(
            ss.Result('efficacy', dtype=float, label='Efficacy'),
            ss.Result('effective_value', dtype=float, label='Effective value')  # The effective value of the parameter that this intervention modulates.
        )
    ...
    def _get_target_val_arr(self, sim, idx):
        """Get target value of an attribute that is an iterable"""
        attr = sim
        for attr_name in self.target_attr_path[:-1]:
            attr = getattr(attr, attr_name)
        target_attr = self.target_attr_path[-1]
    ...
    def step(self):
        ...
        if sim.now('year') >= self.start and len(self.time):
...
class environmental_exposure_reduction(WASH):
    ...
    this intervention modifies the model/module parameter.
    """
    def __init__(self, **kwargs):
        super().__init__(**kwargs)
        return
...
class environmental_trapezoidal_modulation(WASH):
    ...
    def step(self):
        ...
        self.results['effective_value'][sim.ti] = self.target_baseline
        if sim.now('year') >= self.start and len(self.time):
            time_days = sim.now('year') * days_per_year
...
class infectiousness_clearence(ss.Product):
    ...
    def administer(self, uids):
        ...
        clearence = sim.people.typhoid.infectiousness[uids] * (self.pars.clearence_rate / day2year) * sim.pars.dt
...
class vaccination_with_waning(ss.RoutineDelivery):
    ...
    def step(self):
        ...
        """
        if self.start_year <= sim.now('year') <= self.end_year:
            ...
            if len(new_accept_uids):
                ...
                self.ty_vaccinated[new_accept_uids] = sim.now('year')
            if len(self.vaccinated.uids):
                current_year = sim.now('year') * np.ones(len(self.vaccinated.uids))
...
class blocking_vaccine(ss.Product):
    ...
    the agent can't acquire an infection, if the level is 1.0, it can acquire the infection -- also
