...
def digitize_ages_1yr(ages):
    ...
    (0, ss.time.default_start_year). The bin index is used as an integer representation
    of the agent's age.
    """
    # Create 1y age bins because ppl.age is a continuous variable
    age_cutoffs = np.arange(0, ss.time.default_start_year)
...
def process_gallstone_data(df, coi="prob"):
    ...
    Returns: data, type depends on dataset
       arr(`numpy.ndarray`): array data of size (ss.time.default_start_year x 2)
    """

    complete_df = pd.DataFrame({
        'age': np.concatenate(
            [np.arange(lo, hi if pd.notnull(hi) else ss.time.default_start_year) for lo, hi
             in df[['age_lo', 'age_hi']].values]),
        coi: np.repeat(df[coi].values,
                       df['age_hi'].fillna(ss.time.default_start_year).sub(
