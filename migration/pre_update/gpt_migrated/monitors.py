...
__all__ = ["StatesConsistencyMonitor", "HistogramsByAgeSexMonitor"]
...
class HistogramsByAgeSexMonitor(Monitor):
    """
    ...
    reporting_rate (emod parameter) is used for 60% blood culture sensitivity
    and 75% healthcare seeking. By default, scaling=1.0, as if we had
    ...
    def init_pre(self, sim):
        ...
        if self.aggregate_time is None or self.aggregate_time == "subsample":
            ...
            self.stock_ntpts = len(sc.inclusiverange(self.record_from, self.record_until, sim.t.dt))  # number of time points for the internal stock arrays
            ...
            if "path" not in specs:
                ...
                for sex in sexes:
                    self.stocks += [ss.Result(f"{sex}_{attrlbl}",
                                               shape=(self.stock_ntpts, self.nags), dtype=res_dtype,
                                               scale=False, label=f"{sex}_{reslbl}"), ]

                    self.results += [
                        ss.Result(f"{sex}_{attrlbl}",
                                  shape=(self.ntpts, self.nags), dtype=res_dtype,
                                  scale=True, label=f"{sex}_{reslbl}"), ]

        self.results += [ss.Result("yearvec", shape=(self.ntpts,),
    ...
    def set_observation_interval(self, sim):
        """ Set the correct endpoints of the observation period recorded by this monitor"""
        if self.record_from is None and self.record_until is None:
            start_year = sim.t.start
            stop_year = sim.t.stop
        elif self.record_from is not None and self.record_until is None:
            start_year = self.record_from if not self.record_from < sim.t.start else sim.t.start
            stop_year = sim.t.stop
        elif self.record_from is None and self.record_until is not None:
            start_year = sim.t.start
            stop_year = self.record_until if not self.record_until > sim.t.stop else sim.t.stop
        else:
    ...
    def _default_sampling(self, sim):
        if sim.t.ti % self.monitor_step == 0:
            self._apply(sim)
            self.ti += 1
        return
    ...
    def apply(self, sim):
        if self.record_from <= sim.t.now('year') <= self.record_until:
            self.sample(sim)
        return
    ...
    def plot_waterfall(self, key=None, max_timepoints=16, fig=None, style='fancy', fig_kw=None, plot_kw=None):
        ...
        The function generates uses kernel density estimation to visualize the data. If there's no data for the
            ...
            for ax, (key, res) in zip(axs, flat.items()):
                ...
                for idx, ti in enumerate(t_indices):
                    ...
                    try:
                        ...
                        ax.bar(self.age_bin_centers, data_ti, bottom=y_scaling * idx, color='#2f72de', alpha=0.3,
                               edgecolor="white")
...
class StatesConsistencyMonitor(Monitor):
    ...
    def apply(self, sim):
        """
        Checks states that should be mutually exclusive and collectively exhaustive
        """
