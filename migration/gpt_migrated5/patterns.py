...
class Pattern:
    ...
    variable like age or time, or 'agent-space'. Based on starsim's Dist class
...
class StateVariable(ss.BaseArr):
    ...
    def __init__(self, module=None, name=None, shape=None, dtype=None, scale=None):
        super().__init__()
        self.name = name
        self.module = module
        self.scale = scale
        self.init_values(shape=shape, dtype=dtype)
        return
    ...
    def to_df(self):
        return sc.dataframe({self.name: self.values})
