...
class EnvironmentalPool(ss.Demographics):
    def __init__(self, pars=None, metadata=None, **kwargs):
        ...
        self.define_pars(
            ...
            decay_rate=ss.perday(0.3),  # Decay rate of environmental in fraction of CFUs that decay per day
            volume=1,              # Assumed volume of the environmental pool. Units: to be defined.
            acceptable_level=600,  # CFU/volume, usually expressed in CFU/ml (not used at the moment)
            transmission=ss.Pars(
                rel_trans=1e-3,     # Long-cycle exposure (to the environment) multiplier
                shedding_rate=ss.rate(1.0, unit='day'),  # Rate at which infectious people shed CFUs to the environment
                env2ppl_exposure_rate=ss.poisson(lam=2.0),  # Poisson rate for daily exposures via environment route
            ),
        )
    ...
    def init_env_pool(self, sim):
        ...
        self.sv.cfu_conc[sim.t.ti] = self.pars.init_cfu   # Fill initial conditions
        return
    ...
    def step(self):
        ...
        effective_rate = p.decay_rate.to('year')  # transform to yearly rate
