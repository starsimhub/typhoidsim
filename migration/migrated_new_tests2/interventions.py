...
__all__ = [
    ...
    'base_test',
    # Treatments
    'acute_treatment', 'infection_clearance', 'vaccination_with_waning',
    # Environmental Interventions
    'shedding_reduction', 'environmental_cleanup', 'environmental_exposure_reduction',
    'environmental_seasonality', 'environmental_trapezoidal_modulation',
    # Behavioral Change
    'behavioral_change',
    # Products
    'infectiousness_redux', 'infectiousness_clearance', 'blocking_vaccine',
    'typhoid_vaccine'
]
...
class acute_treatment(ss.Intervention):
    ...
    def init_pre(self, sim):
        super().init_pre(sim)
        self.define_results(
            ss.Result('n_treated', sim.t.npts, dtype=int)
        )
...
class infection_clearance(ss.Intervention):
    ...
    def init_pre(self, sim):
        super().init_pre(sim)
        self.define_results(
            ss.Result('n_treated', sim.t.npts, dtype=int, label="Were treated"),
            ss.Result('n_started_tr', sim.t.npts, dtype=int, label="Started treatment")
        )
        self.initialized = True
        return
    ...
    def step(self):
        ...
        if len(cleared_uids):
            self.clear_infection(cleared_uids)
        return
    ...
    def clear_infection(self, cleared_uids):
...
class base_test(ss.Intervention):
    ...
    def init_pre(self, sim):
        super().init_pre(sim)
        self.define_results(
            ss.Result('new_positive', sim.t.npts, dtype=int, label="New Positive"),
            ss.Result('new_tested', sim.t.npts, dtype=int, label="New Tested"),
            ss.Result('positivity', sim.t.npts, dtype=float, label="Positivity")
        )
    ...
    def step(self):
        ...
        self.results['positivity'][self.sim.ti] = sc.safedivide(self.results['new_positive'][self.sim.ti], self.results['new_tested'][self.sim.ti])
        return tested_uids
...
class WASH(ss.Intervention):
    ...
    def init_pre(self, sim):
        ...
        self.define_results(
            ss.Result('efficacy', sim.t.npts, dtype=float),
            ss.Result('effective_value', sim.t.npts, dtype=float)
        )
...
class environmental_trapezoidal_modulation(WASH):
    ...
    due to crop irrigation, health inspections of food vendors.
    """
...
class environmental_seasonality(ss.Intervention):
    ...
    def init_pre(self, sim):
        ...
        self.time = sc.inclusiverange(0, self.dur, sim.t.dt)
        self.define_results(
            ss.Result('seasonal_cfu', len(self.time), dtype=float, label="Seasonal CFUs")
        )
        self.initialized = True
        return
    ...
    def step(self):
        if self.sim.year >= self.start and len(self.time):
            ...
            self.sim.demographics['environmentalpool'].sv.cfu_conc[self.sim.ti-1] = (val + seasonal_cfu) / self.sim.demographics['environmentalpool'].pars.volume
...
class vaccination_with_waning(ss.RoutineDelivery):
    ...
    def init_pre(self, sim):
        ...
        if self.debug:
            self.define_results(
                ss.Result('immunity', shape=(sim.t.npts, sim.pars["n_agents"]), dtype=float, label="Acquired Immunity")
            )
        return
