...
that only subsample and/or aggregate simulated data (monitors), and
...
class HistogramsByAgeSexMonitor(Monitor):
    ...
    def init_pre(self, sim):
        ...
        if self.aggregate_time in aggregation_functions:
        ...
        for attrname, specs in self.to_record.items():
            if "path" not in specs:
                raise ValueError(f"Will not be able to record {attrname} because 'path' is "
                                 f"missing in the `to_record` configuration dictionary.")
    ...
    def step(self):
        if self.sim.year >= self.record_from and (self.sim.year <= self.record_until):
