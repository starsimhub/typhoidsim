...
def digitize_ages_1yr(ages):
    ...
    (0, ss.defaults.max_age). The bin index is used as an integer representation
    of the agent's age.
    """
...
def get_data_home(data_home=None):
    ...
    specified by the `STARSSIM_DATA` environment variable (if it exists)
    or otherwise it will use the default data directory.
    """

    if data_home is None:
        data_home = ss.settings.options.data_home
