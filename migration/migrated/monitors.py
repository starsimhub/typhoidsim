...
class histograms_by_age_sex_monitor(Monitor):
    ...
    def init_pre(self, sim):
        ...
        if self.aggregate_time is None or self.aggregate_time == "subsample":
            ...
            self.stock_ntpts = len(sc.inclusiverange(self.record_from, self.record_until,
                                                     sim.t.dt))  # number of time points for the internal stock arrays
    ...
    def step(self):
        if self.now('year') >= self.record_from and (self.now('year') <= self.record_until):
            self.sample(self.sim)
        return

    def finalize_results(self):
        for stock_name in self.stocks:
            self.results[stock_name][:] = self.aggregate(
                self.stocks[stock_name][:]) if self.agg_func is not None else self.stocks[stock_name][:]
        ...
        with sc.options.with_style(style):
            ...
            for ax, (key, res) in zip(axs, flat.items()):
                for tidx in sorted(t_index):
                    ax.bar(np.arange(0, len(self.age_bin_centers)), res[tidx, :], **plot_kw,
                           label=f"t={yearvec[tidx]:.4f}", alpha=0.2)
