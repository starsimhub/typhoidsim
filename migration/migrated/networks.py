...
class CommunityNet(ss.DynamicNetwork):
    ...
    def __init__(self, pars=None, key_dict=None, **kwargs):
        ...
        self.define_states(
            ss.IndexArr('age_group', default=0, dtype=ss_int_, label='Age group')
        )
        self.age_group_size = None
        return

    def get_contact_rates(self):
        """ Get average number of total contacts per day, per age group and matrix of proportions. """
        contact_rate_matrix = self.pars.age_mixing['matrix']
        total_rate = contact_rate_matrix.sum(axis=1)
        total_rate[total_rate == 0.0] = 1.0
...
class HouseholdNet(ss.DynamicNetwork):
    ...
    def get_households(self):
        """ Calculate households based on size distribution and location data """
...
def generate_household_head_by_size_distribution(nhh_by_hs, hh_head_age_dist):
    ...
    df = pd.DataFrame(np.outer(hh_size_dist, hh_head_age_dist[:, 1]) * n_households,
                      columns=hh_head_age_dist[:, 0], index=np.arange(len(hh_size_dist)))
    return df
