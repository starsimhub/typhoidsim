...
__all__ = [
    'get_data_home', 'load_dataset', 'get_dataset_names', 'get_attr_vals', 'digitize_ages_1yr',
    'test_cpu_performance', 'generate_unique_filename', 'to_df', 'promotetoiterable', 'generate_age_bin_labels'
]
...
def digitize_ages_1yr(ages):
    ...
    of the agent's age.
    """
    age_cutoffs = np.arange(0, ss.defaults.max_age)
...
def load_dataset(ds_name, data_home=None, **kwargs):
    ...
    This function provides access to a collection of datasets
    that are useful for setting empirical distributions rather than hardcoding them.

    The datasets are expected to be simple tabular data saved in csv
    files. This function may apply some preprocessing but is not intended to be a full ingest and preprocessing pipeline.
...
def remove_empty_rows(df):
    # Check for empty rows -- a common problem with csv files
...
def process_gallstone_data(df, coi="prob"):
    ...
    Returns: data, type depends on dataset
       arr(`numpy.ndarray`): array data of size (ss.defaults.max_age x 2)
    """

    complete_df = pd.DataFrame({
        'age': np.concatenate(
            [np.arange(lo, hi if pd.notnull(hi) else ss.defaults.max_age) for lo, hi
             in df[['age_lo', 'age_hi']].values]),
        coi: np.repeat(df[coi].values,
                          df['age_hi'].fillna(ss.defaults.max_age).sub(
...
def to_df(sim, sep='_'):
    """
    Export results as a Pandas dataframe. Available in newer starsim versions (i.e., 2.0)
    Only saves 1D result arrays, discards the results from analyzers, unless they
    are 1D arrays.
    """
    sim.check_results_ready('Please run the sim before exporting the results')
