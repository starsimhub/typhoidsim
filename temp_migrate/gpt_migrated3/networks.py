...
__all__ = ["CommunityNet", "HouseholdNet", "adjust_households", "generate_household_head_by_size_distribution"]
...
class CommunityNet(ss.DynamicNetwork):
    ...
    def get_contact_rates(self):
        ...
        contact_rate_probs = contact_rate_matrix / total_rate[:, None]
    ...
    def get_age_groups(self):
        ...
        self.age_group[:] = tyu.digitize_ages(
            self.sim.people.age[:],
            np.concatenate([self.pars.age_mixing['age_lb'], self.pars.age_mixing['age_ub'][-1:]])
        )

        people = self.sim.people
        born = people.alive & (people.age > 0)

        self.age_group_size = np.bincount(
            self.age_group[born.uids],
    ...
    def get_contacts(self, born, n_contacts):
        ...
        idx1, idx2 = np.triu_indices(len(available_uids), k=1)

        probs = self.age_mix_matrix_probs * self.age_group_size[:, None]
        edge_probs = probs[self.age_group[available_uids[idx1]], self.age_group[available_uids[idx2]]]
    ...
    def plot_age_mixing_density(self, to_plot=None):
        ...
        kde = self.estimate_age_mixing_density(to_plot=to_plot) / self.age_group_size[:, None]
...
class HouseholdNet(ss.DynamicNetwork):
    ...
    def __init__(self, pars=None, key_dict=None, **kwargs):
        ...
        self.update_pars(pars, **kwargs)
        self.dist = ss.randint(name='HouseholdNet')  # Default RNG
    ...
    def get_source(inds, n_contacts):
        ...
        count = 0
        source = np.zeros((total_number_of_half_edges,), dtype=ss_int)
    ...
    def get_contacts(self, inds, n_contacts):
        ...
        adjusted_number_of_contacts = np.round(number_of_contacts / 2).astype(ss_int)
    ...
    def add_pairs(self):
        ...
        number_of_contacts = sc.randround(number_of_contacts / 2).astype(ss_int)
    ...
    def get_households(self):
        ...
        Returns:
            An array with the count of households of size 's'.
        """
        ...
        household_size_dist = ss.choice(p=hhs_props, a=hhs_bins, name='Household size distribution', strict=False)

        av_household_size = np.sum(hhs_bins * hhs_props)
        n_households = np.round(requested_n_agents / av_household_size)
        n_households_by_size = np.zeros(len(hhs_bins), dtype=ss_int)
        ...
        while n_agents_diff != 0:
            ...
            n_agents_diff, n_households_by_size = adjust_households(n_agents_diff, n_households_by_size, new_household_size)
...
def adjust_households(delta_agents, nhh_by_hs, delta_hh_size):
    ...
    delta_agents += sign * delta_hh_size
    nhh_by_hs[delta_hh_size - 1] += sign
...
def generate_household_head_by_size_distribution(nhh_by_hs, hh_head_age_dist):
    ...
    Returns:
        pd.DataFrame: Counts of unique combinations of household heads of a given age and household size.
    """
    import pandas as pd
    n_households = np.sum(nhh_by_hs)
    ...
    df = pd.DataFrame(
        np.outer(hh_size_dist, hh_head_age_dist[:, 1]) * n_households,
        columns=hh_head_age_dist[:, 0], index=hh_size_dist
    )
