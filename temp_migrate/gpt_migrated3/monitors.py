...
class HistogramsByAgeSexMonitor(Monitor):
        ...
        for attrname, specs in self.to_record.items():
            if "path" not in specs:
                ...
                if self.aggregate_sex:
                    ...
                    sexes = ["f", "m"]
                for sex in sexes:
                    self.stocks += [ss.Result(f"{sex}_{attrlbl}", label=f"{sex}_{reslbl}",
                                              shape=(self.stock_ntpts, self.nags), dtype=res_dtype,
                                              scale=False, timevec=self.yearvec), ]

                    self.results += [
                        ss.Result(f"{sex}_{attrlbl}", label=f"{sex}_{reslbl}",
                                  shape=(self.ntpts, self.nags), dtype=res_dtype,
                                  scale=True, timevec=self.yearvec), ]

        self.results += [ss.Result("yearvec", label="Calendar years (float representation)",
                                   shape=(self.ntpts,), dtype=float, scale=False, timevec=self.yearvec), ]
    ...
    def step(self, sim):
        if sim.t.year >= self.record_from and (sim.t.year <= self.record_until):
            self.sample(sim)
    ...
    def plot_waterfall(self, key=None, max_timepoints=16, fig=None, style='fancy', fig_kw=None, plot_kw=None):
        ...
        The function uses kernel density estimation to visualize the data. If there's no data for the
