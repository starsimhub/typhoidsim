...
@nb.njit(cache=True)
def digitize_ages_1yr(ages, max_age):
    """
    This function returns the indices of the 1-year age bins in the range
    (0, max_age). The bin index is used as an integer representation
    of the agent's age.
    """
    age_cutoffs = np.arange(0, max_age)
...
def load_dataset(ds_name, data_home=None, **kwargs):
    ...
    that are useful to set empirical distributions (e.g., demographics)
    rather than having those hardcoded in the code.
