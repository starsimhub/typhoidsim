...
def customize_aliases(mod_name='typhoidsim', json_path='objects.json'):
    """
    Manually add aliases to functions, so instead of e.g. typhoidsim.typhoid.Typhoid, you can link via typhoidsim.Typhoid (and therefore ty.Typhoid)
    """
    ...
    for item in items:
        ...
        if len(parts) < 3 or parts[0] != mod_name:
            continue
        objname = parts[2] # e.g. 'Typhoid' from typhoidsim.typhoid.Typhoid
...
def build_objects_inv(json_path='objects.json', inv_path='objects.inv'):
    ...
    so other projects can resolve Typhoidsim references via intersphinx.
    """
    import sphobjinv as soi
    sc.heading('Building Sphinx objects.inv ...')
    data = sc.loadjson(json_path)
    inv = soi.Inventory()
    inv.project = data.get('project', 'typhoidsim')
    inv.version = str(data.get('version', ty.__version__))
