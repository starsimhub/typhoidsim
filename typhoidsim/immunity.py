...
def imm_decay_by_age(age_bins=None, vals=None):
    ...
    Args:
        ...
        vals (list | np.array): List of immunity decay values, expressed in years,  for each age bin. Length n-1
    ...
    vals = sc.promotetoarray(vals) if vals is not None else np.array([505.27, 505.27, 505.27, 505.27])/tyd.days_per_year  # Decay time constant, in years, one value per age bin of interest Maximum protection at t=0 of receiving a vaccine
