...
# Default datatypes, same as Startsim
dtypes = sc.objdict(
    bool=bool,
    int=np.int64,
    float=np.float64,
...
class Options(sc.objdict):
    ...
    def __init__(self):
        ...
        self.setattribute('optdesc',
                          optdesc)  # Set the description as an attribute, not a dict entry
        self.setattribute('orig_options',
                          sc.dcp(opts))  # Copy the default options
        return
    ...
    def get_orig_options():
        ...
        options.data_home = os.getenv('TYPHOIDSIM_DATA', data_home)
    ...
    def __exit__(self, *args, **kwargs):
        ...
        try:
            ...
            errormsg = 'Please use ty.options.context() if using a with block'
